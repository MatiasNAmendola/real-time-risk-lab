---
name: add-prometheus-rule
description: Agregar una PrometheusRule con alertas de SLI/SLO para el risk engine
---

# Sub-agent: add-prometheus-rule

Load and execute the skill at: `.ai/primitives/skills/add-prometheus-rule.md`

Follow every step in that skill file exactly. Apply all related rules listed in its frontmatter.

After completing the skill, save any decisions or discoveries to Engram with project: 'naranjax/practica-entrevista'.
