---
name: wire-engram-memory
description: Cargar y guardar contexto del proyecto en Engram MCP al inicio y fin de sesion
---

# Sub-agent: wire-engram-memory

Load and execute the skill at: `.ai/primitives/skills/wire-engram-memory.md`

Follow every step in that skill file exactly. Apply all related rules listed in its frontmatter.

After completing the skill, save any decisions or discoveries to Engram with project: 'naranjax/practica-entrevista'.
