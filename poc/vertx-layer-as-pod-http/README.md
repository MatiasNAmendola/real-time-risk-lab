...
One PoC in the Real-Time Risk Lab portfolio. Same risk evaluation domain as the other variants, different architecture.
...
client -> controller (8080) -> usecase (8081) -> repository (8082)
                   x-pod-token: ${CONTROLLER_TO_USECASE_TOKEN:-change-me-controller-usecase-token}
                                           x-pod-token: ${USECASE_TO_REPOSITORY_TOKEN:-change-me-usecase-repository-token}
```
...
| Dimension | vertx-layer-as-pod-http | vertx-layer-as-pod-eventbus |
|---|---|---|
...
```bash
# Build fat-jar
./gradlew :poc:vertx-layer-as-pod-http:shadowJar
...
|---|---|---|---|
| controller-pod | 8080 | 8180 | avoids clash with vertx-layer-as-pod-eventbus |
| usecase-pod | 8081 | 8181 | |
| repository-pod | 8082 | 8182 | |

UIDs: controller=1101, usecase=1102, repository=1103 (1001-1004 reserved for vertx-layer-as-pod-eventbus).
