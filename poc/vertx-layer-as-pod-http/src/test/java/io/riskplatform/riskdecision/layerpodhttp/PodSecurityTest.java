package io.riskplatform.riskdecision.layerpodhttp;

import io.riskplatform.riskdecision.layerpodhttp.common.PodSecurity;
import io.riskplatform.riskdecision.layerpodhttp.repository.RepositoryPod;
import io.riskplatform.riskdecision.layerpodhttp.usecase.UsecasePod;
...
/**
 * Token-based permission tests. Deploys usecase-pod + repository-pod on dynamic ports once per class.
 *
 * Key invariant tested:
 *   - controller token (${CONTROLLER_TO_USECASE_TOKEN:-change-me-controller-usecase-token}) is accepted by usecase, rejected by repository
 *   - usecase token (${USECASE_TO_REPOSITORY_TOKEN:-change-me-usecase-repository-token}) is accepted by repository
 */
...
class PodSecurityTest {
    ...
    static void setUp() throws InterruptedException {
        ...
        vertx.deployVerticle(new RepositoryPod(repositoryPort))
                .compose(__ -> vertx.deployVerticle(new UsecasePod(usecasePort, repositoryPort)))
