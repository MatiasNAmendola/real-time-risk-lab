package io.riskplatform.riskdecision.monolith.controller;

import com.fasterxml.jackson.databind.ObjectMapper;
import io.riskplatform.riskdecision.monolith.usecase.MonolithEventBusAddress;
...
/**
 * HTTP server verticle for the vertx-monolith-inprocess PoC.
 *
 * <p>Exposes the same API surface as the vertx-layer-as-pod-eventbus controller-app
...
public class HttpServerVerticle extends AbstractVerticle {
    ...
    public void start(Promise<Void> startPromise) {
        ...
        router.get("/healthz").handler(ctx ->
            ctx.response().setStatusCode(200)
                .putHeader("Content-Type","application/json")
                .end(new JsonObject().put("status","UP").put("service","vertx-monolith-inprocess").encode()));
