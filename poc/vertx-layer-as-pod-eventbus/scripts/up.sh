...
COMPOSE_ARGS=(-f "$REPO_ROOT/compose/docker-compose.yml"
              -f "$REPO_ROOT/poc/vertx-layer-as-pod-eventbus/compose.override.yml")
...
for app in "${APPS[@]}"; do
  jar="$REPO_ROOT/poc/vertx-layer-as-pod-eventbus/$app/build/libs/$app-0.1.0-SNAPSHOT.jar"
  if [[ ! -f "$jar" ]]; then
  ...
  if (( size < 1048576 )); then
    ...
    "$REPO_ROOT/gradlew" -p "$REPO_ROOT" \
      ":poc:vertx-layer-as-pod-eventbus:$app:shadowJar" --rerun-tasks \
...
RUNTIME_INPUTS=(
  ...
  "$REPO_ROOT/poc/vertx-layer-as-pod-eventbus/compose.override.yml"
  "$REPO_ROOT/poc/vertx-layer-as-pod-eventbus/hazelcast/hazelcast.xml"
)
RECREATE_LIST=()
for app in "${APPS[@]}"; do
  jar="$REPO_ROOT/poc/vertx-layer-as-pod-eventbus/$app/build/libs/$app-0.1.0-SNAPSHOT.jar"
  newest_runtime=$(_newest_epoch "$jar" "${RUNTIME_INPUTS[@]}" \
    "$REPO_ROOT/poc/vertx-layer-as-pod-eventbus/$app/Dockerfile")
...
mkdir -p "$OUT_DIR"
{
  ...
  echo "    Tansu broker      : tcp://localhost:9092 (compose maps 9092:9092)"
  echo "    OpenObserve        : http://localhost:5080  (admin@example.com / ${OPENOBSERVE_PASSWORD:-change-me-openobserve-local})"
  echo ""
