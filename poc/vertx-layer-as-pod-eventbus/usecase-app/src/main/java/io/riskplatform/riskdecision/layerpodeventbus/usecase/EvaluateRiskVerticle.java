package io.riskplatform.riskdecision.layerpodeventbus.usecase;

import com.fasterxml.jackson.databind.ObjectMapper;
import io.riskplatform.riskdecision.layerpodeventbus.shared.EventBusAddress;
...
import io.riskplatform.riskdecision.layerpodeventbus.shared.resilience.LatencyBudget;
import io.riskplatform.riskdecision.layerpodeventbus.shared.resilience.SimpleCircuitBreaker;
import io.riskplatform.riskdecision.layerpodeventbus.shared.rules.FraudRule;
import io.riskplatform.riskdecision.layerpodeventbus.shared.rules.HighAmountRule;
import io.riskplatform.riskdecision.layerpodeventbus.shared.rules.NewDeviceYoungCustomerRule;
import io.riskplatform.riskdecision.layerpodeventbus.shared.rules.RuleEvaluation;
...
import io.riskplatform.riskdecision.layerpodeventbus.usecase.aws.S3AuditPublisher;
import io.riskplatform.riskdecision.layerpodeventbus.usecase.aws.SqsDecisionPublisher;
...
public class EvaluateRiskVerticle extends AbstractVerticle {
    ...
    public void start(Promise<Void> startPromise) {
        ...
        vertx.<Void>executeBlocking(() -> {
            ...
            .compose(v -> vertx.eventBus()
                ...
                .completion())
            .compose(v -> vertx.eventBus()
                .<String>consumer(EventBusAddress.USECASE_READY, this::handleReady)
                .completion())
            .onSuccess(v -> {
    ...
    private void handleReady(Message<String> msg) {
        DeliveryOptions opts = new DeliveryOptions().setSendTimeout(1_500L);
        vertx.eventBus().<String>request(EventBusAddress.REPOSITORY_READY, "ping", opts)
            .onSuccess(reply -> msg.reply("READY(repository=" + reply.body() + ")"))
            .onFailure(err -> msg.fail(503, "repository not ready: " + err.getMessage()));
    }
    ...
    private void continueEvaluation(Message<String> msg, RiskRequest req,
        ...
        vertx.eventBus().<String>request(
            ...
            if (ruleEngine != null) {
                io.riskplatform.rules.engine.FeatureSnapshot featureSnap =
                        io.riskplatform.rules.engine.FeatureSnapshot.builder()
                ...
                ruleResults = engineDecision.ruleResults().stream()
                        .filter(io.riskplatform.rules.rule.RuleEvaluation::triggered)
