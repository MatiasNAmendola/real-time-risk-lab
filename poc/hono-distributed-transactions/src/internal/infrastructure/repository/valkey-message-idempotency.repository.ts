...
export class ValkeyMessageIdempotencyRepository implements MessageIdempotencyRepository {
  ...
  private client(): Redis {
    this.redis ??= new Redis({ host: process.env.VALKEY_HOST ?? 'localhost', port: Number(process.env.VALKEY_PORT ?? 6379), maxRetriesPerRequest: null });
    return this.redis;
  }

  private key(idempotencyKey: string): string { return `eda:idempotency:${idempotencyKey}`; }
}
