...
@Injectable()
export class BullMqEdaService implements OnModuleInit, OnModuleDestroy {
  private readonly logger = new Logger(BullMqEdaService.name);
  private readonly queueName = process.env.EDA_QUEUE_NAME ?? 'risk-domain-events';
  ...
  constructor(@Inject(MESSAGE_IDEMPOTENCY_REPOSITORY) private readonly idempotency: MessageIdempotencyRepository) {}

  onModuleInit(): void {
    if (process.env.EDA_WORKER_ENABLED !== 'true') return;
    this.queue();
    this.worker = new Worker<DomainMessage, EdaProcessResult>(
      this.queueName,
      async (job) => this.processMessage(job),
      { connection: this.connection },
    );
    this.worker.on('completed', (job, result) => this.logger.log({ jobId: job.id, result }, 'EDA message processed'));
    this.worker.on('failed', (job, error) => this.logger.error({ jobId: job?.id, error: error.message }, 'EDA message failed'));
  }
