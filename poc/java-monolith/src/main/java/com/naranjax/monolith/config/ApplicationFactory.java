package com.naranjax.monolith.config;

import com.naranjax.monolith.controller.HttpServerVerticle;
...
import com.naranjax.monolith.repository.SecretsBootstrap;
...
import com.naranjax.monolith.usecase.EvaluateRiskUseCase;
 ...
 *
 * <p>Contrast with java-vertx-distributed where each layer lives in a separate JVM
...
public class ApplicationFactory {
    ...
    public Future<Void> deploy() {
        ...
        return vertx.executeBlocking(() -> {
            ...
            String dbName     = System.getenv().getOrDefault("PG_DATABASE", "naranjax");
            String dbUser     = System.getenv().getOrDefault("PG_USER",     "naranjax");
            ...
            return vertx.deployVerticle(usecaseVerticle)
                .compose(id -> {
                    log.info("[java-monolith] EvaluateRiskUseCase deployed: {}", id);
                    return vertx.deployVerticle(httpVerticle);
                })
                .compose(id -> {
                    log.info("[java-monolith] HttpServerVerticle deployed: {}", id);
