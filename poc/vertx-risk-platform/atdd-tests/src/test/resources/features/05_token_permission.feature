...
Feature: Token-based permission model
  ...
  #   CONTROLLER_TO_USECASE_TOKEN = "controller-risk-evaluate-token"  scope: risk:evaluate
  #   USECASE_TO_REPOSITORY_TOKEN = "usecase-repository-rw-token"     scope: repository:rw
  #
  ...
  Scenario: repository-pod blocks controller token (least-privilege enforcement)
    # Controller owns "controller-risk-evaluate-token" for risk:evaluate scope.
    # Repository only accepts "usecase-repository-rw-token" for repository:rw scope.
    ...
    And header x-pod-token = 'controller-risk-evaluate-token'
    And header x-pod-scopes = 'risk:evaluate'
  ...
  Scenario: repository-pod accepts usecase token with correct scope
    Given url 'http://localhost:8182'
    Given path '/internal/idempotency/find'
    And header x-pod-token = 'usecase-repository-rw-token'
