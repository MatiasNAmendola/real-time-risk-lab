...
class SmartTruncator:
    ...
    def detect_command_type(self, hint: str, output: str) -> str:
        ...
        if re.search(r"pytest|jest|junit|./gradlew test|go test|npm test|bun test|bun run test|cargo test", combined):
            return "test"
        if re.search(r"\b./gradlew\b.*build|gradle|npm run build|bun run build|cargo build|go build|make\b", combined):
            return "build"
