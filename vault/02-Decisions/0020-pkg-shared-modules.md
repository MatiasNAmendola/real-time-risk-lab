...
The `pkg/*` layer es la response un this: un set de purpose-built shared modules bajo Gradle que ambos PoCs (and eventually production services) can depend on. These son no application modules — they son libraries con no framework dependencies, publishable a un Maven repository.
...
Define each shared concern como un separate Gradle subproject bajo `pkg/`. All subprojects use `naranja.library-conventions` convention plugin (from `build-logic/`). Each module has un single responsibility y minimal dependencies en other `pkg/*` modules (DAG, no cycles). Modules son publishable un Maven Local via `./gradlew publishToMavenLocal` para consumption por Maven PoCs.
...
### Opción D: Maven BOM con shared parent POM
- **Ventajas**: Consistent con la PoC build tool; standard Java monorepo pattern.
- **Desventajas**: Parent POM inheritance has la brittleness problems described en ADR-0019; Gradle provides better module isolation; Maven BOM import cannot be consumed por Gradle's native catalog sin `platform()` dependency type.
- **Por qué no**: Gradle es la elegida build tool para `pkg/*` (ADR-0019). Mixing Maven BOM con Gradle version catalog would require maintaining two version management files.
...
### Negativo
- Maven PoCs (bare-javac, Vert.x) must publish `pkg/*` un Maven Local (`./gradlew publishToMavenLocal`) antes de `mvn` builds can resolve la dependencies — un two-step build process.
- Modules en `pkg/*` currently have no test coverage para algunos concerns (e.g., `pkg:config`) — tracked como un gap.

### Mitigaciones
- `BUILDING.md` documents la two-step build order: `./gradlew publishToMavenLocal` antes de `mvn`.
- CI script runs Gradle first, then Maven, en la correct order.

## Validación

- `./gradlew publishToMavenLocal` publishes todos `pkg/*` JARs un `~/.m2/repository`.
- `cd poc/java-vertx-distributed && mvn clean package` resolves `pkg:events` desde Maven Local sin errors.
