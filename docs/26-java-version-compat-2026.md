...
ADR-0001 establece **Java 21 LTS** como baseline operativo y **Java 25 LTS** como objetivo. La realidad operativa a 2026-05-08 es explícita: **compilamos con `--release 21`, ejecutamos en cualquier JDK 21+ (incluyendo 25)**. Esto NO es regresión de la decisión arquitectónica — es un workaround temporal hasta que el ecosistema de tooling de build catches up.
...
### 4. build-helper-gradle-plugin y otras herramientas

Hay un par de scripts y plugins legacy (algunos de los `pom.gradle-legacy.xml` mantenidos para reverse) que esperan bytecode <= 21. No es bloqueante porque ya no son la build canónica, pero documenta la fricción.

## Decisión

Convention plugin `riskplatform.java-conventions.gradle.kts`:
...
> "Porque en mayo 2026 el ecosistema Gradle todavía no catched up. JMH 1.37, Shadow plugin estable y Karate 1.4 fallan con bytecode 25. El objetivo arquitectónico sigue siendo Java 25 — el workaround es compilar a 21 hasta que las tres herramientas publiquen versiones compatibles. Es exactamente el tipo de tradeoff que diferencia ingeniería pragmática de purismo. Cuando el ecosistema esté listo, es un cambio de una línea."

Pregunta de seguimiento: "¿Y por qué no usás Java 21 LTS directamente entonces, si te conformás con 21 en bytecode?"

Respuesta:

> "Porque las objetivos de **runtime** apuntan a Java 25: virtual threads escaling, GC mejoras, FFM API. El baseline verificable corre con JDK 21+ y bytecode 21; Java 25 es el objetivo para runtime cuando el tooling lo permita hasta que las tools soporten 25. Si bajara la decisión completa a 21 LTS, sería renunciar a las features runtime que aprovechamos hoy."
...
| build-helper-gradle-plugin | 3.x | parcial | No bloqueante |
